## This code should be used when you want to simulate the model using quadratic-form utility
...
## Utility parameters
parameter <- utilitypar.quad ## utilitypar or abcdef or IVpar of par.sign or utilitypar.quad
...
datalie2 <- datalie2 %>% 
  dplyr::mutate(utility2 = parameter[1] + parameter[2]*所要時間 + parameter[3]*費用 + parameter[4]*I(費用^2)
                + parameter[5]*log(Freq) + parameter[6]*滞在可能時間 ## 1日の便数ならFreqではなく航空便数!
                + parameter[7]*アクセシビリティ + parameter[8]*鉄道ダミー
                + parameter[9]*エアライン参入数) %>% 
...
prob <- function(a){
  ...
  for(k in 1:nrow(datalie2)){
    exputility[k] <- (a[k]*exp(parameter[1] + parameter[2]*datalie2$所要時間[k] + parameter[3]*Price[k] + parameter[4]*Price[k]^2
                               + parameter[5]*log(Frequency[k]) + parameter[6]*datalie2$滞在可能時間[k]
                               + parameter[7]*datalie2$アクセシビリティ[k] + parameter[8]*datalie2$鉄道ダミー[k]
                               + parameter[9]*datalie2$エアライン参入数[k]))
  }
...
Objective <- function(p){
  ...
  Frequency <- ifelse(Frequency == 0, 1e-4, Frequency)
  for(k in 1:nrow(datalie2)){
    exputility[k] <- (datalie2$alpha[k]*exp(parameter[1] + parameter[2]*datalie2$所要時間[k] + parameter[3]*Price[k] + parameter[4]*Price[k]^2
                                            + parameter[5]*log(Frequency[k]) + parameter[6]*datalie2$滞在可能時間[k]
                                            + parameter[7]*datalie2$アクセシビリティ[k] + parameter[8]*datalie2$鉄道ダミー[k]
                                            + parameter[9]*datalie2$エアライン参入数[k]))
  }
  exputility[c(which(datalie2$airlineNo == n))] <- exputility[c(which(datalie2$airlineNo == n))]*exp(-parameter[3]*Price[c(which(datalie2$airlineNo == n))] -parameter[4]*Price[c(which(datalie2$airlineNo == n))]^2)
  exputility[c(which(datalie2$airlineNo == n))] <- exputility[c(which(datalie2$airlineNo == n))]*exp(parameter[3]*p + parameter[4]*p^2) ## ここでパラメータ負に入っているのが効いているのでは？
  sumexputility <- data.frame(i = datalie2$i, j = datalie2$j, exputility = exputility)
  sumexputility <- sumexputility %>% 
    dplyr::group_by(i, j) %>% 
    dplyr::mutate(sumexputility = sum(exputility)) %>% 
