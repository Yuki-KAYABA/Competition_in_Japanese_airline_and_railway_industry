...
## Read data
airline2 <- read.csv("C:/Users/kayab/yuki/‘åŠwu‹`/‘åŠw‰@ƒ[ƒ~/Data/airline2.csv", header = T, skip = 2, stringsAsFactors = FALSE)
...
airline2$‰^qƒLƒƒ[ƒgƒ‹ <- as.numeric(gsub(",", "", airline2$‰^qƒLƒƒ[ƒgƒ‹))
...
airline2[which(airline2$q‹ó‰ïĞ == "SFJ"), 26] <- 9*DIS ## SFJ‚Íƒ†ƒjƒbƒgƒRƒXƒg‚ª9
airline2[which(airline2$q‹ó‰ïĞ == "SKY"), 26] <- 5.5*DIS ## SKY‚Íƒ†ƒjƒbƒgƒRƒXƒg‚ª5.5

## Concentrate on Tokyo, Osaka, and Fukuoka
ODnames <- c("“Œ‹", "¬“c", "ŠÖ¼", "‘åã", "_ŒË", "“ì‹I”’•l", "•Ÿ‰ª", "–k‹ãB", "²‰ê")
ODNo <- c(rep(18, 2), rep(25, 4), rep(37, 3))
ODcorrespondence <- data.frame(ODnames = ODnames, ODNo = ODNo)
airline3 <- airline2 %>% 
  dplyr::filter(‹ó`i == "“Œ‹" | ‹ó`i == "¬“c" | ‹ó`i == "ŠÖ¼" | ‹ó`i == "‘åã" | ‹ó`i == "_ŒË" | ‹ó`i == "“ì‹I”’•l" | ‹ó`i == "•Ÿ‰ª" | ‹ó`i == "–k‹ãB" | ‹ó`i == "²‰ê") %>% 
  dplyr::filter(‹ó`j == "“Œ‹" | ‹ó`j == "¬“c" | ‹ó`j == "ŠÖ¼" | ‹ó`j == "‘åã" | ‹ó`j == "_ŒË" | ‹ó`j == "“ì‹I”’•l" | ‹ó`j == "•Ÿ‰ª" | ‹ó`j == "–k‹ãB" | ‹ó`j == "²‰ê")
for(k in 1:2){
...
## Read data
airline4 <- read.csv("C:/Users/kayab/yuki/‘åŠwu‹`/‘åŠw‰@ƒ[ƒ~/Data/airline.csv", header = T)

## Replace airports name with correct ones
airline4$‹ó`i <- as.character(airline4$‹ó`i)
airline4$‹ó`j <- as.character(airline4$‹ó`j)
airline4$‹ó`i <- gsub("‰H“c", "“Œ‹", airline4$‹ó`i)
airline4$‹ó`j <- gsub("‰H“c", "“Œ‹", airline4$‹ó`j)
airline4$‹ó`i <- gsub("ˆÉ’O", "‘åã", airline4$‹ó`i)
airline4$‹ó`j <- gsub("ˆÉ’O", "‘åã", airline4$‹ó`j)
airline4$‹ó`i <- gsub("ŠÖ¼‘Û", "ŠÖ¼", airline4$‹ó`i)
airline4$‹ó`j <- gsub("ŠÖ¼‘Û", "ŠÖ¼", airline4$‹ó`j)
...
airline3$q‹ó‰ïĞ <- as.character(airline3$q‹ó‰ïĞ)
airline4$‹ó`i <- as.character(airline4$‹ó`i)
airline4$‹ó`j <- as.character(airline4$‹ó`j)
airline4$q‹ó‰ïĞ <- as.character(airline4$q‹ó‰ïĞ)

write.csv(airline4, "C:/Users/kayab/yuki/‘åŠwu‹`/‘åŠw‰@ƒ[ƒ~/Data/airline4.csv")
...
airlinenames <- c("JAL", "ANA", "SFJ", "SKY", "IBX", "JAC", "ƒWƒFƒCEƒGƒA", "AMX")
airlineNo <- c(seq(1:8))
...
colnames(airline3)[30] <- "Š—vŠÔ"
colnames(airline3)[31] <- "”ï—p"
colnames(airline3)[32] <- "q‹ó•Ö”"
colnames(airline3)[33] <- "‘Øİ‰Â”\ŠÔ"
colnames(airline3)[34] <- "ƒGƒAƒ‰ƒCƒ“Q“ü”"
...
colnames(airline3)[38] <- "ƒAƒNƒZƒVƒrƒŠƒeƒB"

## Write csv
write.csv(airline3, "C:/Users/kayab/yuki/‘åŠwu‹`/‘åŠw‰@ƒ[ƒ~/Data/airline3.csv")
...
data2 <- data2 %>% 
  dplyr::mutate(utility = utilitypar[1] + Š—vŠÔ*utilitypar[2] + ”ï—p*utilitypar[3] + log(q‹ó•Ö”)*utilitypar[4] + 
                  ‘Øİ‰Â”\ŠÔ*utilitypar[5] + ƒAƒNƒZƒVƒrƒŠƒeƒB*utilitypar[6] * “S“¹ƒ_ƒ~[*utilitypar[7] + 
                  ƒGƒAƒ‰ƒCƒ“Q“ü”*utilitypar[8]) %>% 
  dplyr::group_by(i, j) %>% 
...
write.csv(data2, "C:/Users/kayab/yuki/‘åŠwu‹`/‘åŠw‰@ƒ[ƒ~/Data/data2.csv")
